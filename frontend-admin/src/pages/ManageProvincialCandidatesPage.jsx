...
import {
  ...
  Search, MoreHorizontal, Camera, FileText, Pencil, Building2, Grid, List, Loader2,
} from "lucide-react";
...
const TABS = [
  { key: "candidates", label: "Candidate Profiles", icon: Users, sublabel: "Provincial registry" },
...
/* ══════════════════════════════════════════════════════════════ */
/*  CANDIDATES PANEL — provincial registry                       */
/* ══════════════════════════════════════════════════════════════ */
function CandidatesPanel({ setMsg }) {
  const { parties } = useParties();
  const { profiles, loading, reload } = useCandidateProfiles({ governmentLevel: "PROVINCIAL" });
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    const payload = { ...form, party_id: form.party_id ? Number(form.party_id) : null, gender: form.gender || null, date_of_birth: form.date_of_birth || null, government_level: "PROVINCIAL" };
    try {
  ...
  return (
    <>
      ...
      <SectionCard style={{ border: `1px solid ${T.borderStrong}` }}>
        <SectionHeader icon={Users} iconColor={T.navy} title="Candidate Profiles" subtitle="Provincial candidate registry — Provincial Assembly" />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", borderRadius: T.radius.md, border: `1px solid ${T.border}`, overflow: "hidden" }}>
              <button onClick={() => setViewMode("grid")} title="Grid view"
                style={{ padding: "6px 10px", background: viewMode === "grid" ? T.accentLight : T.surface, border: "none", borderRight: `1px solid ${T.border}`, cursor: "pointer", color: viewMode === "grid" ? T.accent : T.muted, transition: "all 0.15s" }}>
                <Grid size={15} />
              </button>
              <button onClick={() => setViewMode("list")} title="List view"
                style={{ padding: "6px 10px", background: viewMode === "list" ? T.accentLight : T.surface, border: "none", cursor: "pointer", color: viewMode === "list" ? T.accent : T.muted, transition: "all 0.15s" }}>
                <List size={15} />
              </button>
            </div>
          </div>
                ...
                {filtered.map((c, idx) => {
                  ...
                  return (
                    <tr key={c.id}
                      ...
                      <td style={tdStyle}>
                        {party ? (
                          <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 5, background: T.accentLight, color: T.accent }}>{party.abbreviation}</span>
                        ) : (
                      ...
                      <td style={{ ...tdStyle, textAlign: "center" }}>
                        <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
                          <Btn variant="ghost" small onClick={() => handleStartEdit(c)} style={{ color: T.accent }}><Pencil size={13} /></Btn>
    ...
    >
      ...
      >
        {photo ? (
          ...
          <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${T.navy} 0%, ${T.accent} 100%)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ marginBottom: 8 }}>
          {party ? (
            <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 5, background: T.accentLight, color: T.accent, letterSpacing: "0.03em" }}>{party.abbreviation}</span>
          ) : (
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-around" }}>
          <Btn variant="ghost" small onClick={() => onEdit(candidate)} title="Edit profile" style={{ color: T.accent }}><Pencil size={14} /></Btn>
...
function NominationsPanel({ setMsg }) {
  const { parties } = useParties();
  const { profiles } = useCandidateProfiles({ governmentLevel: "PROVINCIAL" });
  ...
  useEffect(() => { (async () => { try { const d = await listElections(); setElections(d || []); } catch { /* ignore */ } })(); }, []);
