...
export default function ProvincialVoterAssignmentsPage() {
  ...
  const loadAssignments = useCallback(async () => {
    try {
      const data = await listAreaAssignments({ government_level: "PROVINCIAL", page, pageSize: 50 });
  ...
  const loadAreas = useCallback(async () => {
    try {
      const params = { government_level: "PROVINCIAL" };
      if (provinceFilter) params.province_number = Number(provinceFilter);
      const data = await listAreas(params);
  ...
  const loadVoters = useCallback(async (search) => {
    try {
      const data = await listAssignableVotersForArea(search, "PROVINCIAL");
  ...
  const handleAssign = async () => {
    if (!selectedVoter || !selectedArea) {
      setMsg({ type: "error", text: "Select both a voter and a provincial area" });
      return;
    }
    setSubmitting(true);
    try {
      await assignVoterToArea(selectedVoter.id, parseInt(selectedArea, 10), "PROVINCIAL");
  ...
  const confirmRemoveAction = async () => {
    ...
    try {
      await removeAreaAssignment(voterId, "PROVINCIAL");
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <select style={{ ...inp, maxWidth: 220 }} value={provinceFilter} onChange={e => setProvinceFilter(e.target.value)}>
              ...
              {provinces.map(p => (
                <option key={p} value={p}>Province {p} — {PROVINCE_NAMES[p]}</option>
              ))}
            </select>
            <select style={{ ...inp, maxWidth: 450 }} value={selectedArea} onChange={e => setSelectedArea(e.target.value)}>
              <option value="">— Select provincial area —</option>
              {areas.map(a => (
                <option key={a.id} value={a.id}>
                  {a.name || a.title || `Area #${a.id}`}{a.province_number ? ` (P${a.province_number})` : ""}
                </option>
              ))}
            </select>
          </div>
      ...
      <ConfirmDialog open={!!confirmRemove} onClose={() => setConfirmRemove(null)}
        onConfirm={confirmRemoveAction} title="Remove Provincial Assignment"
