/**
 * OtpInput — Shared 6-digit one-time-code input component.
 *
...
/* ── Styles (aligned with admin-auth theme) ──────────────────── */
...
export default function OtpInput({
  ...
  useEffect(() => {
    if (autoFocus && refs.current[0]) {
      // Small delay so the DOM settles (e.g. conditional render)
  ...
  function handleChange(idx, e) {
    ...
    if (!raw) return; // non-numeric or cleared — handled by onKeyDown
    const next = [...digits];
    next[idx] = raw[0]; // take first digit
    emit(next);
    // advance focus
    if (idx < NUM_DIGITS - 1) {
    ...
    >
      {Array.from({ length: NUM_DIGITS }, (_, i) => (
        <input
          ...
          maxLength={2}           /* allow typing to trigger onChange */
          value={digits[i]?.trim() || ""}
